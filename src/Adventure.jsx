...
const DEFAULT_IMAGE = 'https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=560&q=75&auto=format&fit=crop';
...
export default function Adventure() {
  ...
  const [bookingData, setBookingData] = useState({ date: new Date().toISOString().split('T')[0], scheduleId: '', participants: 1 });
  ...
  const getAdventureForTile = (col, row) => {
    if (adventures.length === 0) return null;
    const idx = Math.abs((col * 73856093) ^ (row * 19349663)) % adventures.length;
  ...
  const getTileImage = (adventure) => {
    const src = adventure?.imageUrl || DEFAULT_IMAGE;
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let col = c0; col <= c1; col++) {
        ...
        for (let row = r0; row <= r1; row++) {
          ...
          const adv = getAdventureForTile(col, row);
          const tile = getTileImage(adv);
          ...
          ctx.arcTo(dx + dw, dy, dx + dw, dy + RADIUS, RADIUS);
          ctx.lineTo(dx + dw, dy + dh - RADIUS);
          ctx.arcTo(dx + dw, dy + dh, dx + dw - RADIUS, dy + dh, RADIUS);
          ctx.lineTo(dx + RADIUS, dy + dh);
          ctx.arcTo(dx, dy + dh, dx, dy + dh - RADIUS, RADIUS);
          ...
          if (hp > 0.01 && adv) {
            ...
            ctx.fillStyle   = '#fff';
            ctx.font        = `600 13px "Helvetica Neue",sans-serif`;
            ctx.fillText(adv.name, tx, ty + TILE_H + 22);
            ctx.globalAlpha = hp * 0.5;
            ctx.font        = `400 11px "Helvetica Neue",sans-serif`;
            ctx.fillText(adv.category + " - $" + adv.price, tx, ty + TILE_H + 38);
            ctx.restore();
          }
        }
      }
    ...
    function onWheel(e) {
      if (!interactable || selectedAdv) return;
      e.preventDefault();
      targetX += e.deltaX;
      targetY += e.deltaY;
    }
    ...
    function onMouseUp(e) {
      if (!drag) {
        ...
        if (hovCol !== null && hovRow !== null) {
          const adv = getAdventureForTile(hovCol, hovRow);
          if (adv) handleOpenBooking(adv);
        }
      }
      drag = false;
    }

    const handleOpenBooking = async (adv) => {
      setSelectedAdv(adv);
      try {
      ...
      if (tx >= 0 && tx <= TILE_W) {
        ...
        if (ty >= 0 && ty <= TILE_H) {
          ...
          if (adv) {
            ...
            tooltip.style.left     = (e.clientX + 14) + 'px';
            tooltip.style.top      = (e.clientY + 14) + 'px';
            tooltip.textContent    = "Book " + adv.name;
          }
          return;
        }
      }
    ...
    gsap.set(lines, { opacity: 0, y: 40 });
    const tl = gsap.timeline({ delay: 0.8 });
    tl.to(lines, { opacity: 1, y: 0, duration: 0.8, stagger: 0.1, ease: 'power3.out' });
    tl.to(lines, { opacity: 0, y: -40, duration: 0.8, stagger: 0.1, ease: 'power3.inOut', onComplete: () => { if (textEl) textEl.style.display = 'none'; }}, '+=0.9');
    tl.to(overlay, { opacity: 0, duration: 1.2, ease: 'power2.inOut', onComplete: () => { if (overlay) overlay.style.display = 'none'; }}, '+=0.3');
    tl.to(canvas, { opacity: 1, duration: 1.2, ease: 'power2.out' }, '<');
    tl.to(introProxy, { scale: 1, y: 0, duration: 2.5, ease: 'expo.out', onUpdate() { targetY = introProxy.y; }}, '<');
    tl.call(() => { interactable = true; targetY = 0; offsetY = 0; });
    tl.to(navEl, { opacity: 1, duration: 0.5, ease: 'power2.out' }, '-=0.4');
  ...
  }, [adventures, selectedAdv]);

  useEffect(() => {
    if (bookingData.scheduleId && bookingData.date) {
      adventureService.getAvailability(bookingData.scheduleId, bookingData.date)
        .then(setAvailability)
        .catch(() => setAvailability(null));
    }
  }, [bookingData.scheduleId, bookingData.date]);
  ...
  return (
    <div className="adventure-root">
      ...
      <div ref={overlayRef} className="adventure-overlay" />
      <div ref={textRef} className="adventure-textlayer">
        <span ref={line1Ref} className="adventure-line">UNFORGETTABLE</span>
        <span ref={line2Ref} className="adventure-line">MOMENTS</span>
        <span ref={line3Ref} className="adventure-line">AWAIT.</span>
      </div>
      <div ref={tooltipRef} className="adventure-tooltip" />
      ...
      {selectedAdv && (
        <div className="booking-modal-overlay">
          <div className="booking-modal">
            <button className="close-btn" onClick={() => setSelectedAdv(null)}>×</button>
            <h3>Book {selectedAdv.name}</h3>
            <p>{selectedAdv.description}</p>
            <form onSubmit={handleBook} className="modal-form">
              ...
              <select value={bookingData.scheduleId} onChange={(e) => setBookingData({...bookingData, scheduleId: e.target.value})} required>
              ...
              {availability !== null && <p>Remaining spots: {availability}</p>}

              <label>Participants:</label>
              <input type="number" min="1" value={bookingData.participants} onChange={(e) => setBookingData({...bookingData, participants: e.target.value})} required />
